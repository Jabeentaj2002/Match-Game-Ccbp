...
class App extends Component {
  ...
  onRenderGame = () => {
    ...
    return (
      <div>
        <img src={activeImage.imageUrl} className="large-image" alt="match" />
        <ul className="tabs-list">
          {tabsList.map(tab => (
            <li className="list-item" key={tab.tabId}>
              <button
                className={`tab-buttons ${
                  activeTabId === tab.tabId ? 'active-btn' : ''
                }`}
                onClick={() => this.onChangeTab(tab.tabId)}
              >
                {tab.displayText}
              </button>
            </li>
        ...
        <ul className="thumbnails-container">
          {filteredList.map(eachItem => (
            <li className="thumbnail-list-item" key={eachItem.id}>
              <button
                type="button"
                className="thumbnail-btn"
                onClick={() => this.onThumbNailClicked(eachItem.id)}
              >
                <img
  ...
  render() {
    const {timeRemaining, score, isGameInProgress} = this.state
        ...
        <nav className="nav-container">
          ...
          <div className="score-time-container">
            ...
            <ul className="nav-items">
              ...
              <li>
                <p className="secs">{timeRemaining} sec</p>
              </li>
            </ul>
          </div>
        </nav>
        <div className="game-container">
          {isGameInProgress ? this.onRenderGame() : this.onRenderResultCard()}
        </div>
      </div>
    )
  }
